...
public class Comun_usuarios extends Comun_usuarios_ventana implements Serializable, View {
	...
	public Comun_usuarios() {
		...
		if (Administrador.isAdministrador(usu))
	...
	public void cargarPartesTema(Label nomT, Label usuC, Button borrarT, Label ultMen, Label diaT, Label numCom,
		...
		if(!Administrador.isAdministrador(usu)) borrarT.setVisible(false);
	...
	public void cargarPartesMensaje(Label contI, Label usuC, Button borrarMensaje, Label temaPertenece, Label diaM,
		...
		if(!Comun_privilegiados.isPrivilegiado(usu)) borrarMensaje.setVisible(false);
		if(usu==null) 
		...
		mensaje1.addLayoutClickListener(new LayoutClickListener() {
			@Override
			public void layoutClick(LayoutClickEvent event) {
				MyUI.navigator.navigateTo("seccion/"+men.getSon_de().getPertenece_a().getORMID()+"/tema/"+men.getSon_de().getORMID());
			}
		});
	}
