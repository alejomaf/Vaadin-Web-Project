...
public class DB_Main implements iCorreo, iAdministrador, iUsuario_Registrado, iComun_usuarios, iUsuario_No_Registrado, iUsuario_propietario_mensaje, iModerador, iComun_privilegiados, iComun_registrados, Serializable {
	...
	public void Eliminar_mensaje(int aID) {
		try {
			com.mds.database.Media med=men.cogerMedia(men.getMensaje(aID));
			if(med!=null) {
			if(getFoto(med)!=null) {
				men.eliminarFoto(getFoto(med));
			}else if(getVideo(med)!=null) {
				men.eliminarVideo(getVideo(med));
			}
//			men.eliminarMedia(med);
			}
	...
	public void crearMensaje(int aID, int idR, int idT, String aMensaje, String foto, String video) {
		try {
			men.crearMensaje(aID, idR, idT, aMensaje);
			com.mds.database.Mensaje menAux;
			if(foto!=null||video!=null) {
				menAux=men.cogerUltimoMensaje();
				if(foto!=null) men.crearFoto(menAux, foto);
				if(video!=null) men.crearVideo(menAux, video);
			} 
	...
	@Override
	public Mensaje[] Cargar_mensajes_usuario(int aID) {
		try {
			return men.getMensajesUsuarios(aID);
		} catch (PersistentException e) {
			e.printStackTrace();
		}
		return null;
	...
	@Override
	public Temas[] cargarTodosLosTemas(int aID) {
		try {
			return tem.cargarTodosLosTema(aID);
		} catch (PersistentException e) {
			e.printStackTrace();
		}
		return null;
	...
	@Override
	public void ocultarTema(Temas tema) {
		try {
			tem.ocultarTema(tema);
		}catch(PersistentException e) {
			e.printStackTrace();
		}
	}
	...
	@Override
	public void mostrarTema(Temas tema) {
		try {
			tem.mostrarTema(tema);
		}catch(PersistentException e) {
			e.printStackTrace();
		}
	}
	...
	@Override
	public Media getMedia(com.mds.database.Mensaje menAux) {
		try {
			return men.cogerMedia(menAux);
		} catch (PersistentException e) {
			e.printStackTrace();
		}
		return null;
	...
	@Override
	public Foto getFoto(Media medAux) {
		try {
			return men.cogerFoto(medAux);
		} catch (PersistentException e) {
			e.printStackTrace();
		}
		return null;
	...
	@Override
	public Video getVideo(Media medAux) {
		try {
			return men.cogerVideo(medAux);
		} catch (PersistentException e) {
			e.printStackTrace();
		}
		return null;
