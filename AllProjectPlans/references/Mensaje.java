...
public class Mensaje extends Mensaje_ventana{
	...
	public Mensaje(com.mds.database.Mensaje men) {
		...
		med= iadm.getMedia(men);
		
		if(med!=null) {
			fot=iadm.getFoto(med);
			vid=iadm.getVideo(med);
		
		if(fot!=null) {
			
			imagen.setVisible(true);
			FileResource fotoMensajeAux= new FileResource(new File(fot.getEnlace()));
			imagen.setSource(fotoMensajeAux);
			
		}else if(vid!=null) {
			
			videoAdjunto.setVisible(true);
			videoAdjunto.setResource(new ExternalResource(vid.getEnlace()));
			videoAdjunto.setTargetName("_blank");
		}
}
	}
