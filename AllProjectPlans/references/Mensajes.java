...
public class Mensajes extends Mensajes_ventana implements View {
	...
	public Mensajes(com.mds.database.Secciones sec, com.mds.database.Temas tem) {
		...
		if(Comun_privilegiados.isPrivilegiado(usu)) {
			borrarTema.setVisible(true);
		}
	...
	public void usuarioNoRegistrado() {
		...
		if(tem.getPublico()) {
			...
			mostrarTema.setVisible(true);
			mostrarTema.addClickListener(new Button.ClickListener() {
			public void buttonClick(ClickEvent event) {
				icp.mostrarTema(tem);
				Page.getCurrent().reload();
			}
	...
	public void asignarValores() {
		...
		if(usu!=null) {
		if(Usuario_propietario_mensaje.isPropietario(usu, men)) mensaje=new Mensaje_propio(men);
